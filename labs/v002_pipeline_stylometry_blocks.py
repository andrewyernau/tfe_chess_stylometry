#!/usr/bin/env python3
"""
Pipeline de Estilometría con FRAGMENTACIÓN EN BLOQUES (Versión 002).

CAMBIOS PRINCIPALES V002:
- Bloques de 5 movimientos para mejor visualización
- 15 jugadas por jugador (30 movimientos totales) divididos en 3 bloques
- Heatmaps en escala de grises (negro=rápido, blanco=lento)
- 6 imágenes por partida: 3 tableros + 3 heatmaps
- Verifica PGNs existentes antes de extraer
...
class ChessStylometryPipelineV002:
    ...
    def run(self) -> Dict:
        ...
        """
        print(f"\n{'#'*70}")
        print(f"# PIPELINE DE ESTILOMETRÍA V002 - BLOQUES")
        print(f"{'#'*70}\n")
    ...
    def _generate_images_parallel(self, valid_players: Dict) -> Dict:
        ...
        """
        from v002_generate_images_blocks import generate_images_blocks
        ...
        for i, player_name in enumerate(valid_players.keys()):
            ...
            print(f"      Tableros: {stats['boards_created']} "
                  f"({stats['boards_created']//len(self.blocks)} partidas × {len(self.blocks)} bloques)")
            print(f"      Heatmaps: {stats['heatmaps_created']}")
            
            # Verificar sincronización
            if stats['boards_created'] != stats['heatmaps_created']:
                print(f"      ⚠ Desincronización: boards={stats['boards_created']}, "
